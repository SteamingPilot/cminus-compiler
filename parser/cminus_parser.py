...
from api import *
from astnode import TypeCheckingVisitor, CompileVisitor
...
def p_program_procs(attr): 
  'program : procedures'
  # Since there is no vList, we have add an empty node
...
def p_procedure_decl(attr):
  ...
  # The attr[1] holds() a tuple of: (type, ID, paramDeclList<D_pl>, varDeclList<D_vl>)
  attr[0] = makeFdecl(None, attr[1][0], attr[1][1], attr[1][2], attr[1][3], attr[2])

def p_procedure_head_decl(attr):
  'procedureHead : functionDecl declList'
  # The attr[1] holds a list of: (type, ID, paramDeclList<D_pl>)
  # The attr[2] holds a varDeclList<D_vl>)
...
def p_procedure_head(attr):
  ...
  # The attr[1] holds a list of: (type, ID, paramDeclList<D_pl>)
  attr[0] = attr[1]
  attr[0].append(makeVlist(None, None))
...
def p_function_decl_noparam(attr):
  ...
  # Since there is no paramenter, we have to add an empty $plist node
...
def p_write_expr(attr):
  ...
  # Example:
  # 1+2 = addExp PLUS MulExp -> addExp -> simpleExp -> expr. Using the expr we will create the $write node
...
def parse_program(fn):
  ...
  # parsing the program, and getting the root ASTNode 
  ...
  # return  getTreeNodeRep(root)
  return root

def compile_program(fn):
  print(fn)
  root = parse_program(fn)
  ...
  compiler.generateAssembly(asm_fn)
